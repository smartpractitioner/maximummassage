...
**CMP: build our own, not buy (decided 2026-07-14).** Supersedes the earlier "Enzuzo as factory default" plan. Driven by factory economics (~$5/domain/mo × N clients, forever) and by a verified legal check: Google's certified-CMP requirement is **publisher-scoped and EEA/UK/Switzerland-scoped**, so it does not apply to us as Canadian *advertisers* — meaning a DIY banner is fully viable. Full design + the core-functionality spec: [`docs/phase-7-diy-cmp-design.md`](phase-7-diy-cmp-design.md). [`docs/sop-cmp-comparison.md`](sop-cmp-comparison.md) stays as the **buy-option fallback** (Enzuzo) if maintenance burden outgrows the savings or a client's jurisdiction demands certification. **Note: MH needs no cookie banner at all** (Alberta, informed implied consent per the playbook) — the CMP is a Phase 7 factory capability needed at the first non-Alberta client, and must not land on the launch critical path.
